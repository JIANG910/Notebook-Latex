...
\end{equation}
3.\enspace$f(x)=\sin x$
\vspace{0.8em} \\ \solve $f'(x)=\lim\limits_{h\to 0}\di\frac{f(x+h)-f(x)}{h}=\lim\limits_{h\to 0}\di\frac{\sin (x+h)-\sin x}{h}=\lim\limits_{h\to 0}\di\frac{1}{h}\cdot 2\cos\di(x+\frac{h}{2})\cdot\sin\di\frac{h}{2}$\\  
\hspace*{7em} =$\lim\limits_{h\to 0}\frac{\di\sin\frac{h}{2}}{\di\frac{h}{2}}\cdot\cos(x+\di\frac{h}{2})\cdot\di\sin\frac{h}{2}=\cos x$
\\即
\vspace{0.8em}
\begin{equation}
	\nonumber
...
\vspace{0.8em} \\ \solve $f'(x)=\lim\limits_{h\to0}\di\frac{f(x+h)-f(x)}{h}=\lim\limits_{h\to0}\di\frac{\log_a (x+h)-\log_a x}{h}=\lim\limits_{h\to0}\di\frac{1}{h}\cdot\log_a(1+\di\frac{h}{x})=\di\frac{1}{h}\cdot\lim\limits_{h\to0}\di\frac{x}{h}\cdot\log_a(1+\di\frac{h}{x})$\\
令$u=\di\frac{h}{x},h\to0,u\to0$,得
\begin{equation}
	\nonumber f'(x)=\frac{1}{x}\cdot\lim\limits_{h\to0}\frac{x}{h}\cdot\log_a(1+\frac{h}{x})=\frac{1}{x}\cdot\lim\limits_{u\to0}\frac{1}{u}\cdot\log_a(1+u)=\frac{1}{x}\cdot\lim\limits_{u\to0}\log_a(1+u^{\frac{1}{u}})=\frac{1}{x}\cdot\lim\limits_{u\to0}\log_a e=\frac{1}{x}\cdot\frac{1}{\ln a}=\frac{1}{x\cdot\ln a}
	\end{equation}
即\\
\begin{equation}
	\nonumber
...
\subsection{导函数与单侧导数}
\tdefination[导数定义2]
如果函数$y=f(x)$在开区间$I$内的每点处都可导，那么就称函数$f(x)$在开区间$I$内可导。这时，对于任意$xz\in I$,都对应着$f(x)$的一个确定的导数值。这样就构成了一个新的函数，这个函数叫做原来函数的导函数，简称导数。记作
\begin{equation}
	\nonumber
...
双曲余弦函数 \kg ch $x=\di\frac{e^x+e^{-x}}{2}$  \kg \kg 反双曲余弦函数\kg arch $x=\ln(x\pm\sqrt{x^2+1})\left\{\begin{aligned}
		...
		\end{aligned}\right.
		$\vspace*{1em}\\
双曲正切函数 \kg th $x=\di\frac{e^x-e^{-x}}{e^x+e^{-x}}$  \kg \kg 反双曲正切函数\kg arch $x=\di\frac{1}{2}\ln\di\frac{1+x}{1-x}$\vspace*{1em}\\
...
&3.\enspace[f(x)\cdot g(x)]'=f'(x)\cdot g(x)+f(x)\cdot g'(x) &4.&\enspace[\frac{f(x)}{g(x)}]'=\frac{f'(x)\cdot g(x)-f(x)\cdot g'(x)}{[g(x)]^2}&&
\end{flalign*}
\subsection{反函数的求导法则}
...
\begin{flalign*}
	...
	&3.\enspace\d(Cu)=C\d u &4.&\enspace\d(\frac{u}{v})=\frac{v\d u-u\d v}{v^2}&&
\end{flalign*}
...
\end{equation}
\kg 有些时候对于复杂的显函数也可以用隐函数求导的方法进行求导，常见的方法就是\highlight{red}{\index{DSQDF@对数求导法}对数求导法}，下面给出两个典型例题：\\
...
\begin{equation}
	\nonumber
	\left\{ \begin{aligned}
		&\enspace x_0=a\cos \frac{\pi}{4}=\frac{\sqrt{2}a}{2}\\
		&\enspace y_0=b\cos \frac{\pi}{4}=\frac{\sqrt{2}b}{2}\\
	\end{aligned}\right.
\end{equation}
曲线在$M_0$的切线斜率为:
\begin{equation}
...
\begin{equation}
	(a^x)^{(n)}=a^x\cdot\ln^na(a>0,a\neq1)\stackrel{a=\e}{\longrightarrow}(\e^x)^{(n)}=\e^x
	\end{equation}
\begin{equation}
	{[(a+bx)^{\mu}]}^{(n)}=
...
\\ \kg 设函数$u=u(x),v=vv(x)$在点$x$处具有$n$阶导数，
\begin{equation}
	\nonumber
...
\theorem[罗尔(Rolle)定理]
设函数$f(x)$满足\\
(1)\enspace 在闭区间$[a,b]$上连续；\\
(2)\enspace 在开区间$[a,b]$内可导；\\
(3)\enspace 在区间端点处的函数值相等，即$f(a)=f(b)$,\\
